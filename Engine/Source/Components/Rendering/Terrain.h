...
struct AutoMeshRule
{
	...
	{
		RaylibModel* model = nullptr;
		Material* material = nullptr;
		float weight = 1.0f;
	};
	std::vector<CachedVariant> cachedVariants;
};
...
public:
	...
	// Auto Mesh Placement
	void AddAutoMeshRule(const std::string& name, int targetLayerIndex);
	void RemoveAutoMeshRule(int ruleIndex);
	AutoMeshRule* GetAutoMeshRule(int index);
	const AutoMeshRule* GetAutoMeshRule(int index) const;
	int GetAutoMeshRuleCount() const { return static_cast<int>(autoMeshRules.size()); }

	void GenerateAutoMeshes();
	void RegenerateAutoMeshes();
	void ClearAutoGeneratedMeshes();
	void LoadAutoMeshRuleModels(AutoMeshRule& rule);
	MeshVariant* SelectWeightedVariant(AutoMeshRule& rule);
	bool CheckPlacementConstraints(const AutoMeshRule& rule, int x, int z, float terrainHeight, const Vector3& normal) const;
...
private:
	...
	std::vector<AutoMeshRule> autoMeshRules;
	std::map<int, std::vector<int>> autoGeneratedMeshIndices;
