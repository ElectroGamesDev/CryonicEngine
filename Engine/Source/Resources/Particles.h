...
public:
...
#endif
		auto it = particles.find(relativePath);
		if (it != particles.end())
			jsonData = &it->second;
		else
		{
			...
			if (!file.is_open())
			{
				ConsoleLogger::ErrorLog("Particles failed to load. Path: " + fullPath);
				return;
			}
			nlohmann::json data;
			file >> data;
			particles[relativePath] = data;
			jsonData = &particles[relativePath];
		}
	}
	...
	static std::unordered_map<std::string, nlohmann::json> particles;
